...
export default async function (req, res) {
  ...
  if (animal.trim().length === 0) {
    res.status(400).json({
      error: {
        message: "Please enter a valid bassline",
      }
    });
    return;
  }
...
function generatePrompt(bassline) {
  return `Suggest a line or lines that would harmonize nicely with the following Bassline, following the rules of traditional 18th century counterpoint (4th species counterpoint). It is given in Guido Music Notation:
